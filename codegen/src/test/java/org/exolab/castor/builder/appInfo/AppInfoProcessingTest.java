 ...
 * @since 1.2.1
 */
public class AppInfoProcessingTest {
    ...
    public final void testAppInfoProcessingWithEntitySchema() throws Exception {
        _xmlSchema = getClass()
            .getResource("schema-entity.xsd")
            .toExternalForm();
        InputSource inputSource = new InputSource(_xmlSchema);
        _generator.generateSource(
            inputSource,
            getClass().getPackage().getName() + ".generated.entity"
        );
        ...
        for (int i = 0; i < cInfos.size(); ++i) {
            ...
            if (xmlNature.getNodeName().equals("book")) {
                 ...
                 */
                assertFalse(
                    cInfo.hasNature(JDOClassInfoNature.class.getName())
                );
     ...
     * @throws Exception if an error occurs
     */
    public final void testAppInfoProcessingWithEntityAndSolrjSchema()
        throws Exception {
        _xmlSchema = getClass()
            .getResource("schema-solrj.xsd")
            .toExternalForm();
        InputSource inputSource = new InputSource(_xmlSchema);
        _generator.generateSource(
            inputSource,
            getClass().getPackage().getName() + ".generated.solrj"
        );
        ...
        for (int i = 0; i < cInfos.size(); ++i) {
            ...
            if (xmlNature.getNodeName().equals("book")) {
                ...
                assertTrue(
                    fInfo.hasNature(SolrjFieldInfoNature.class.getName())
                );
                SolrjFieldInfoNature solrjNature = new SolrjFieldInfoNature(
                    fInfo
                );
    ...
    public final void testAppInfoProcessingNoJDOContent() throws Exception {
        ...
        _generator.generateSource(
            inputSource,
            getClass().getPackage().getName() + ".generated.noJDO"
        );
    ...
    public final void testAppInfoProcessingWithComplexContent()
        throws Exception {
        _xmlSchema = getClass()
            .getResource("complex-content.xsd")
            .toExternalForm();
        InputSource inputSource = new InputSource(_xmlSchema);
        _generator.generateSource(
            inputSource,
            ...
            if (xmlNature.getNodeName().equals("person")) {
                ...
                for (int j = 0; j < fInfos.length; ++j) {
                    ...
                    if (xmlFieldNature.getNodeName().equals("ssn")) {
                        ...
                        xmlFieldNature.getNodeName().equals("firstName")
                    ) {
                        assertEquals("firstName", columnName);
                        assertEquals("varchar", columnType);
                    } else if (
                        xmlFieldNature.getNodeName().equals("lastName")
                    ) {
                        assertEquals("lastName", columnName);
                        assertEquals("varchar", columnType);
                    } else if (
                        xmlFieldNature.getNodeName().equals("birthdate")
                    ) {
