...
public abstract class AbstractProperties {
    ...
    protected AbstractProperties(
        ...
        final ClassLoader domain
    ) {
        _applicationClassLoader = (app != null)
            ? app
            : getClass().getClassLoader();
        _domainClassLoader = (domain != null)
            ? domain
            : getClass().getClassLoader();
    ...
    protected void loadDefaultProperties(
        ...
        if (!inCastorJar && !inJavaLibDir) {
            throw new PropertiesException(
                "Failed to load properties: " + filename
            );
        }
    ...
    protected void loadUserProperties(final String filename) {
        ...
        boolean userPropertiesLoaded = loadFromClassPath(
            properties,
            "/" + filename
        );
        ...
        if (!userPropertiesLoaded) {
            userPropertiesLoaded = loadFromWorkingDirectory(
                properties,
                filename
            );
        }
        ...
        if (!userPropertiesLoaded) {
            String property = System.getProperty(
                USER_PROPERTIES_SYSTEM_PROPERTY
            );
            if (property != null && property.length() > 0) {
                File file = new File(property);
                if (file.exists()) {
                    LOG.info(
                        "Loading custom Castor properties from " +
    ...
    private boolean loadFromClassPath(
        ...
        try {
            // ClassLoader.getResource() expects paths without leading slash
            String resourcePath = filename.startsWith("/")
                ? filename.substring(1)
                : filename;
            URL url = _applicationClassLoader.getResource(resourcePath);
            if (url != null) {
            ...
            LOG.warn(
                "Failed to load properties from classpath: " + filename,
                ex
            );
            return false;
        } finally {
            if (classPathStream != null) {
                try {
                    ...
                    LOG.warn(
                        "Failed to close properties from classpath: " + filename
                    );
                }
            }
        }
     ...
     * @return <code>true</code> if properties could be loaded, <code>false</code> otherwise.
     */
    private boolean loadFromJavaHome(
        final Properties properties,
        final String filename
    ) {
        try {
            ...
            return loadFromFile(
                properties,
                new File(new File(javaHome, "lib"), filename)
            );
        } catch (SecurityException ex) {
            LOG.warn(
                "Security policy prevented access to system property 'java.home'.",
                ex
            );
            return false;
        }
    ...
    private boolean loadFromFile(final Properties properties, final File file) {
        try {
            ...
            LOG.warn(
                "Security policy prevented access to properties file: " + file,
                ex
            );
            return false;
        } catch (Exception ex) {
    ...
    public final Boolean getBoolean(final String key) {
        ...
        String msg =
            "Properties value can not be converted to boolean: {0}={1}";
    ...
    public final Integer getInteger(final String key) {
        ...
        if (objectValue == null) {
            ...
            try {
                ...
                Object[] args = new Object[] { key, objectValue };
                String msg =
                    "Properties value can not be converted to int: {0}={1}";
                throw new PropertiesException(
    ...
    public final Class getClass(final String key, final ClassLoader loader) {
        ...
        if (objectValue == null) {
            ...
            try {
                ...
                String msg =
                    "Could not find class of properties value: {0}={1}";
                throw new PropertiesException(
                    MessageFormat.format(msg, args),
                    ex
                );
     ...
     */
    public final Class[] getClassArray(
        final String key,
        ...
        if (objectValue == null) {
            ...
            for (int i = 0; i < classnames.length; i++) {
                try {
                    ...
                    String msg =
                        "Could not find class of properties value: {0}[{1}]={2}";
                    throw new PropertiesException(
                        MessageFormat.format(msg, args),
                        ex
                    );
        ...
        if (objectValue == null) {
            ...
            for (int i = 0; i < classnames.length; i++) {
                ...
                try {
                    ...
                    String msg =
                        "Could not find configured class: {0}[{1}]={2}";
                    throw new PropertiesException(
                        MessageFormat.format(msg, args),
                        ex
                    );
                    ...
                    String msg =
                        "Could not instantiate configured class: {0}[{1}]={2}";
                    throw new PropertiesException(
                        MessageFormat.format(msg, args),
                        ex
                    );
